...
def make_timeline(rows, title="Milestone Timeline", size=(9, None)):
...
def analyse(sheets):
    ...
    def build_theme_charts(theme_name):
        ...
        charts["phase_bar"] = make_hbar(phase_labels, phase_values, phase_colors, title="Phase-wise Progress",
                                         xlabel="% Complete") if (len(phase_labels) > 1 or (phase_labels and phase_labels[0] != "Unassigned")) else None
        charts["timeline"] = make_timeline(milestones, title="Milestone Timeline (Target Date)")
        charts["owner_bar"] = make_hbar(owner_labels, owner_values, [BLUE] * len(owner_labels),
            ...
            if (owner_labels and not (len(owner_labels) == 1 and owner_labels[0] == "Unassigned")) else None
        charts["trend_line"] = make_trend_line(history) if len(history) >= 2 else None
...
<div id="dash">
  ...
  <section class="page on" id="pg-ov">
    ...
    <div class="full cc" id="timeline-card">
      <div class="cc-hd"><span class="cc-dot" style="background:var(--cyan)"></span><span class="cc-title">Milestone Timeline</span></div>
...
function render(d){
  ...
  row2 += upcomingCard(d.upcoming);
  $('ov-row2').innerHTML=row2;

  $('timeline-chart').innerHTML = charts.timeline ? `<img src="${charts.timeline}" class="chart-img" data-chart-key="timeline"/>` : '<div class="empty">No target dates found to build a timeline.</div>';
